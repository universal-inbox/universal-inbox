-- Remove foreign key "on delete" constraints for `user`
...
-- Remove foreign key "on delete" constraints for `integration_connection`
ALTER TABLE integration_connection
  DROP CONSTRAINT integration_connection_user_id_fkey;
...
-- Remove foreign key "on delete" constraints for `third_party_item`
ALTER TABLE notification
  DROP CONSTRAINT fk_source_item,
  ADD CONSTRAINT fk_source_item
    FOREIGN KEY (source_item_id) REFERENCES third_party_item(id);
...
ALTER TABLE task
  ...
  ADD CONSTRAINT fk_sink_item
    FOREIGN KEY (sink_item_id) REFERENCES third_party_item(id);
...
-- Remove foreign key "on delete" constraints for `task`
ALTER TABLE notification
