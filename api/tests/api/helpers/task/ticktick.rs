...
pub async fn mock_ticktick_list_tasks_service(
    ...
    Mock::given(method("GET"))
        .and(path(format!("/project/{project_id}/data")))
        .and(header("authorization", "Bearer ticktick_test_access_token"))
        .respond_with(
            ResponseTemplate::new(200)
                .insert_header("content-type", "application/json")
...
/// Mock TickTick's special `GET /project/inbox/data` endpoint used to fetch
/// tasks stored in the system Inbox (which is not part of the `/project` list).
pub async fn mock_ticktick_inbox_data_service(
    ticktick_mock_server: &MockServer,
    tasks: &[TickTickItem],
) {
    Mock::given(method("GET"))
...
pub async fn mock_ticktick_update_task_service(
    ...
    Mock::given(method("POST"))
        ...
        // Assert the body carries the real projectId (the regression guard — an
        // empty string here used to cause TickTick to duplicate the task) and
