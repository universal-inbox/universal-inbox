        ...
        assert_eq!(response.headers().get("content-security-policy"),
                   Some(
                       &HeaderValue::from_str(
                           &format!(
                               "default-src 'self'; script-src 'self' 'wasm-unsafe-eval' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; object-src 'none'; connect-src 'self' ws://localhost:3003/ http://localhost:3003/ {}; img-src 'self' https://secure.gravatar.com https://avatars.githubusercontent.com https://public.linear.app https://avatars.slack-edge.com data:; worker-src 'none'",
