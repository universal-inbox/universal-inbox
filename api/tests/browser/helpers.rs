...
pub async fn register(page: &Page, app_url: &str, email: &str) {
    ...
    // if the user is auto-logged-in, otherwise the login page will show.
    // Use a short sleep as a fallback since the post-registration redirect target is variable.
    tokio::time::sleep(tokio::time::Duration::from_secs(3)).await;
}

/// Navigate to a page and assert that an expected element is visible.
pub async fn assert_page_loaded(page: &Page, app_url: &str, path: &str, expected_selector: &str) {
    page.goto(&format!("{app_url}{path}"), None)
        .await
