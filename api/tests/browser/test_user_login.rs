...
async fn test_user_can_login(#[future] browser_tested_app: BrowserTestedApp) {
    ...
    // Part B: Navigate to settings page and verify integrations
    page.goto(&format!("{}/settings", app.app_url), None)
        .await
        .expect("Failed to navigate to settings page");
    ...
    // Collect all card title texts to check for expected integration names
    let mut all_titles_text = String::new();
    for i in 0..card_count {
        let card_title = page
            .locator(&format!(
                "div.card.bg-base-200:nth-child({}) div.card-title",
                i + 1
            ))
            .await;
        if let Ok(Some(text)) = card_title.text_content().await {
